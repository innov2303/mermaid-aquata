...
import { sanitizeTvRef } from "../lib/sanitize.js";

const router = Router();
const DATA_FILE = path.join(process.cwd(), "data", "tv-refs.json");
...
router.get("/tv-refs", (_req, res) => {
  res.json(readData());
});

router.post("/tv-refs", adminAuth, (req, res) => {
  const items = readData();
  const sanitized = sanitizeTvRef(req.body);
...
router.put("/tv-refs/:id", adminAuth, (req, res) => {
  const items = readData();
  const idx = items.findIndex((i: any) => i.id === Number(req.params["id"]));
  if (idx === -1) return res.status(404).json({ error: "Non trouvé" });
  const sanitized = sanitizeTvRef({ ...items[idx], ...req.body });
  items[idx] = { ...sanitized, id: items[idx].id };
  writeData(items);
  res.json(items[idx]);
});

router.delete("/tv-refs/:id", adminAuth, (req, res) => {
  let items = readData();
