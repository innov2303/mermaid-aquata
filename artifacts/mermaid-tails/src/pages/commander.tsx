                ...
                <div className={`flex flex-col ${STEP_IMAGES[currentStep] ? 'items-start flex-1' : 'items-center w-full'}`}>
                  ...
                  {'choices' in steps[currentStep] && (steps[currentStep] as { choices: string[] }).choices ? (
                    ...
                    <div className="w-full max-w-lg flex flex-col items-center gap-6 text-center">
                      ...
                      <div className="w-full h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(0,200,239,0.4), transparent)' }} />
                      <p className="text-xs font-light" style={{ color: 'rgba(200,235,255,0.55)' }}>Choisissez votre moyen de contact préféré :</p>
                      <div className="flex flex-col sm:flex-row gap-4 w-full">
                        <a
                          href={INSTAGRAM_URL}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl font-medium text-sm"
                          style={{ background: 'linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045)', color: 'white', boxShadow: '0 0 20px rgba(225,48,108,0.35)', transition: 'box-shadow 0.2s, filter 0.2s' }}
                        ...
                        >
                          ...
                          {(steps[currentStep] as { instagramLabel: string }).instagramLabel}
                        </a>
                        <a
                          href={`mailto:${contactEmail}`}
                          className="flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl font-medium text-sm transition-all duration-200"
                          style={{ background: 'rgba(0,200,239,0.15)', border: '1.5px solid rgba(0,200,239,0.6)', color: '#e0f5ff', boxShadow: '0 0 20px rgba(0,200,239,0.2)', transition: 'box-shadow 0.2s, filter 0.2s', textDecoration: 'none' }}
                          onMouseEnter={e => (e.currentTarget as HTMLElement).style.filter = 'brightness(1.15)'}
                          onMouseLeave={e => (e.currentTarget as HTMLElement).style.filter = ''}
                        >
                          <Send size={18} />
                          {(steps[currentStep] as { formLabel: string }).formLabel}
                        </a>
                      </div>
                    </div>
                  ) : (
